    ...
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 animate-pulse">
      <div className="flex items-center gap-4 w-full">
        ...
        <div className="space-y-2 flex-1">
          ...
          <div className="h-4 bg-gray-100 rounded w-1/6 mt-2" />
        </div>
      </div>
      <div className="h-10 w-24 bg-gray-200 rounded-lg hidden md:block" />
    </div>
  );
};
...
const PendingRequestCard = ({ requestSummary }: { requestSummary: Request }) => {
  ...
  // Use Skeleton for the detail card if loading
...
export default function InterviewsPage() {
  ...
  // Safe access to data for filtering, defaulting to empty arrays if loading/undefined
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap p-1 bg-gray-100 rounded-xl">
          {[
            ...
            { id: "pending", label: `Pending ${!isLoading ? `(${pendingIncoming.length})` : ''}` },
